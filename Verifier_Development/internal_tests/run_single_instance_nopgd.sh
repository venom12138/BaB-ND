...
if [ 0 != ${EXIT_CODE} ]; then
	...
	# run on benchmarks
	START=$(date +%s.%N)
	timeout $KILL_TIMEOUT ./run_instance_nopgd.sh "v1" "$CATEGORY" "$ONNX" "$VNNLIB" "out.txt" "$TIMEOUT"
	...
	if [ 0 != ${EXIT_CODE} ]; then
		...
		if [ 124 == ${EXIT_CODE} ]; then
		    echo "Error: run_instance_nopgd.sh exceeded $KILL_TIMEOUT second timeout!"
		    RESULT_STR="run_instance_nopgd_timeout"
		fi
	else
	...
	echo "run_instance_nopgd.sh exit code: ${EXIT_CODE}, Result: ${RESULT_STR}, Runtime: ${RUNTIME}"
fi
