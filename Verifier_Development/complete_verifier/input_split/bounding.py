...
def get_lower_bound_naive(
        ...
        reference_interm_bounds=None, num_iter=None, split_partitions=2, param_dict=None):
        ...
        if arguments.Config['bab']['branching']['input_split']['ibp_enhancement']:
            ...
            with torch.no_grad():
                ret = self.net.compute_bounds(
                    ...
                    reference_bounds=reference_interm_bounds, param_dict=param_dict
                )
                lb = ret[0]
                lA = None
                lbias = None
                if not param_dict.get("final_ibp", False):
                    lA = _get_lA(ret)
                    lbias = _get_lbias(ret)
...
def get_lower_bound_with_ibp_enhancement(
        ...
        stop_criterion_func=stop_criterion_batch_any, bound_upper=False):
    ...
    lb_crown, ub_crown, A_dict = self.net.compute_bounds(
        x=(x_unverified,), C=C[unverified], method=bounding_method,
        bound_upper=bound_upper, return_A=True, needed_A_dict=needed_A_dict,
        reference_bounds={
