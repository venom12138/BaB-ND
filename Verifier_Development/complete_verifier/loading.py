...
# ANONYMOUS: load vnlib without model
def load_vnnlib(file_root, csv_item):
    onnx_path, vnnlib_path, arguments.Config["bab"]["timeout"] = csv_item
    onnx_path = os.path.join(
        arguments.Config["model"]["onnx_path_prefix"], onnx_path.strip())
    vnnlib_path = os.path.join(
...
def parse_run_mode():
    ...
    # ANONYMOUS: remove file_root in vnnlib_path and onnx_path for multiple runs
    if arguments.Config['specification']['vnnlib_path'] is not None and arguments.Config['specification']['vnnlib_path'].startswith(file_root):
        arguments.Config['specification']['vnnlib_path'] = os.path.relpath(arguments.Config['specification']['vnnlib_path'], file_root)
    if arguments.Config['model']['onnx_path'] is not None and arguments.Config['model']['onnx_path'].startswith(file_root):
        arguments.Config['model']['onnx_path'] = os.path.relpath(arguments.Config['model']['onnx_path'], file_root)
    ...
    if (arguments.Config['general']['csv_name']
        ...
        if arguments.Config['specification']['vnnlib_path'] is None:
            ...
            example_idx_list = [0]
            # ANONYMOUS: add file_root to vnnlib_path
            if file_root and not arguments.Config['specification']['vnnlib_path'].startswith(file_root):
                arguments.Config['specification']['vnnlib_path'] = os.path.join(
                    file_root, arguments.Config['specification']['vnnlib_path'])
