...
class ConfigHandler:
    ...
    def add_common_options(self):
        ...
        # ANONYMOUS: add verbose option
        self.add_argument("--no_verbose", action='store_false', dest='verbose',
                            help='Enable/disable verbose output including the config file, model.',
                            hierarchy=h + ["verbose"])
        ...
        # ANONYMOUS: softmax sampling in input split
        self.add_argument('--input_split_softmax_temperature', type=float, default=0.1,
                          help='The temperature for softmax in input split when sampling domains with bounds.',
                          hierarchy=h + ['softmax_temperature']) 
        # ANONYMOUS: add customized branching heuristic for input split
        self.add_argument("--branching_heuristic", type=str, default='input_split_branching',
                          help='Branching heuristic in input split. Can be the Customized() primitive; refer to custom/custom_branching_heuristic.py for more details.',
                          hierarchy=h + ["branching_heuristic"])
        self.add_argument('--input_split_ub_ratio', type=float, default=0.75,
                          help='The ratio of domains by ranking ub when selecting domains.',
                          hierarchy=h + ['ub_ratio']) 
        ...
        # ANONYMOUS: add pgd_lr_decay mode, pgd_auto_factor
        self.add_argument('--pgd_lr_decay_mode', type=str, default="auto",
                          help='Mode for Learning rate decay factor used in PGD attack. if auto, then determine decay factor based on num_steps and target factor.',
                          hierarchy=h + ["pgd_lr_decay_mode"])
        self.add_argument('--pgd_lr_target', type=float, default=0.05,
                          help='The target learning rate relative to the initial learning rate after decay.',
                          hierarchy=h + ["pgd_lr_target"])
        self.add_argument('--pgd_alpha', type=str, default="auto",
                          help='Step size of PGD attack. Default (auto) is epsilon/auto_factor.',
        ...
        # ANONYMOUS: add pgd_auto_factor
        self.add_argument('--pgd_auto_factor', type=float, default=4,
                          help='Determine step size of PGD attack in auto: epsilon/auto_factor. Default 4.',
                          hierarchy=h + ["pgd_auto_factor"])       
        ...
        # ANONYMOUS: add reject_bad
        self.add_argument("--enable_reject_bad", action='store_true',
                          help='Enable rejecting bad updates in PGD attack.',
                          hierarchy=h + ["reject_bad"])
        ...
        # ANONYMOUS: create new arguments for finding feasible solutions
        # currently, this mode supports input split only, not neuron split
        # some of the arguments are shared with attack and input split and are not repeated here
        h = ["find_feasible_solution"]
        self.add_argument("--enable_find_feasible_solution", action='store_true',
                          help='Enable finding feasible solutions, not verification, not attack.',
                          hierarchy=h + ["enable"])
        self.add_argument("--find_feasible_save_solution", action='store_true',
                          help='Save feasible solutions in file.',
                          hierarchy=h + ["save_solution"])
        self.add_argument("--find_feasible_show_solution", action='store_true',
                          help='Print feasible solutions.',
                          hierarchy=h + ["show_solution"])
        self.add_argument('--find_feasible_report_per_iter',
                          action='store_true',
                          help='Show and save feasible solutions per iteration.',
                          hierarchy=h + ['report_per_iter'])   
        self.add_argument("--enable_report_time", action='store_true',
                          help='Enable report time for branching, bounding and searching.',
                          hierarchy=h + ["report_time"])
        self.add_argument('--save_solution_path', type=str, default='./feasible_sol.txt',
                          help='Save path for feasible solutions.',
                          hierarchy=h + ["save_solution_path"])
        self.add_argument('--save_bab_path', type=str, default='./bab_bounds.pkl',
                          help='Save path for the bounds of every domain in every iteration.',
                          hierarchy=h + ["save_bab_path"])
        self.add_argument("--find_feasible_early_stop_patience", type=int, default=10000,
                          help='Number of iterations of early stop patience for feasible solutions.',
                          hierarchy=h + ["early_stop_patience"])
        self.add_argument('--find_feasible_use_upper_bound', 
                          action='store_true',
                          help='Find feasible solutions using upper bound. helpful for finding feasible solutions for large networks.',
                          hierarchy=h + ['use_upper_bound'])
        self.add_argument('--find_feasible_warm_start', 
                          action='store_true',
                          help='Use previous feasible solutions as warm start for pgd attack.',
                          hierarchy=h + ['warm_start'])
        self.add_argument('--preload_path', type=str, default='',
                          help='Path to load feasible solutions.',
                          hierarchy=h + ["preload_path"])
        self.add_argument('--find_feasible_pre_pgd_steps', type=int, default=5,
                          help="Steps of PGD attack before finding feasible solutions.",
                          hierarchy=h + ["pre_pgd_steps"])
        self.add_argument('--find_feasible_pre_pgd_restarts', type=int, default=5,
                          help="Number of random PGD restarts before finding feasible solutions.",
                          hierarchy=h + ["pre_pgd_restarts"])
        self.add_argument('--find_feasible_sample_with_solution', 
                          action='store_true',
                          help='Sample from feasible solutions. If there are no feasible solutions, sample by softmax.',
                          hierarchy=h + ['sample_with_solution'])
        self.add_argument('--find_feasible_bound_pgd_steps', type=int, default=10,
                          help="Steps of PGD attack to get empirical bounds, only used when bound_method is crown-pgd.",
                          hierarchy=h + ["bound_pgd_steps"])
        self.add_argument('--find_feasible_bound_pgd_restarts', type=int, default=2000,
                          help="Number of random PGD restarts to get empirical bounds, only used when bound_method is crown-pgd.",
                          hierarchy=h + ["bound_pgd_restarts"])
        self.add_argument('--find_feasible_bound_pgd_auto_factor', type=float, default=4,
                          help='Determine step size of PGD attack for bounding: epsilon/auto_factor. Default 4.',
                          hierarchy=h + ["bound_pgd_auto_factor"])   
        self.add_argument('--find_feasible_bound_pgd_lr_decay', type=float, default=0.5,
                          help='Learning rate decay factor used in PGD boudning.',
                          hierarchy=h + ["bound_pgd_lr_decay"])
        self.add_argument("--find_feasible_bound_pgd_interval", type=int, default=-1,
                          help='Call PGD bounding every bound_pgd_interval iterations.',
                          hierarchy=h + ["bound_pgd_interval"])
        self.add_argument("--enable_reject_bad_in_bound", action='store_true',
                          help='Enable rejecting bad updates in PGD bounding.',
                          hierarchy=h + ["reject_bad_in_bound"])
        self.add_argument('--find_feasible_bound_mppi_weight_factor', type=float, default=0.1,
                          help='Determine weight factor of softmax in mppi bounding. Default 0.1.',
                          hierarchy=h + ["bound_mppi_weight_factor"])   
        self.add_argument('--find_feasible_bound_mppi_noise_factor', type=float, default=0.1,
                          help='Determine std of normal noise of MPPI bounding. Default 0.1.',
                          hierarchy=h + ["bound_mppi_noise_factor"])   
        self.add_argument("--fast_pass_bounding", action='store_true',
                          help='Use the bounds during finding feasible solutions as the intermediate bounds and CROWN for final bound.',
                          hierarchy=h + ["fast_pass_bounding"])
        self.add_argument("--final_ibp_bounding", action='store_true',
                          help='Calculate final bound with IBP during finding feasible solutions.',
                          hierarchy=h + ["final_ibp_bounding"])
        self.add_argument("--record_distribution_for_branching", action='store_true',
                          help='Record the distribution of sample to help branching (customized).',
                          hierarchy=h + ["record_distribution_for_branching"])
        self.add_argument('--elite_ratio_for_branching', type=float, default=0.01,
                          help='The ratio of elite samples of the distribution for branching.',
                          hierarchy=h + ["elite_ratio_for_branching"])
        self.add_argument('--min_n_elites_for_branching', type=int, default=10,
                          help='The minimum number of elite samples for branching. Default 10.',
                          hierarchy=h + ["min_n_elites_for_branching"])
        # ANONYMOUS: add customized search method for finding feasible solutions
        self.add_argument('--search_func', type=str, default='None',
                          help='The specific search method for finding feasible solutions.',
                          hierarchy=h + ["search_func"])
        self.add_argument("--no_verbose_search", action='store_false', dest='verbose_search',
                            help='Enable/disable verbose output during searching.',
                            hierarchy=h + ["verbose"])
        self.add_argument("--stuck_patience", type=int, default=-1,
                          help='Number of iterations after best objective stopping improving to adjust the heuristics.',
                          hierarchy=h + ["stuck_patience"])
        self.add_argument("--pruning_stuck_threshold", type=float, default=1,
                          help='The threshold (0-1) to tighten bound estimation. If space size > threshold and no subdomain pruned after some iterations.',
                          hierarchy=h + ["pruning_stuck_threshold"])
        self.add_argument("--pruning_stuck_patience", type=int, default=-1,
                          help='Number of iterations that no subdomain pruned and space size > threshold, then tighten bound estimation.',
                          hierarchy=h + ["pruning_stuck_patience"])
        # ANONYMOUS: add customized early stop condition in crown bounding
        h = ["find_feasible_solution", "early_stop_bounding"]
        self.add_argument("--conditioner", type=str, default='None',
                          help='Customized early stop condition in crown bounding. Support Customized() primitive; refer to custom/custom_early_stop_condition_in_bounding.py for more details.',
                          hierarchy=h + ["conditioner"])
        # ANONYMOUS: add retry_patience when early-stop bounding gives too tight bounds
        self.add_argument("--retry_patience", type=int, default=3,
                          help='Number of iterations to retry when early-stop bounding gives too tight bounds.',
                          hierarchy=h + ["retry_patience"])
        # add config for mppi and cem search
        # only used when search_func is mppi or cem
        # Customized("custom_attacker_sampling", "mppi_attack_with_general_specs")
        # Customized("custom_attacker_sampling", "cem_attack_with_general_specs")
        h = ["find_feasible_solution", "mppi_config"]
        self.add_argument('--n_steps_mppi', type=int, default=8,
                          help="Steps of mppi search.",
                          hierarchy=h + ["n_steps"])
        self.add_argument('--n_samples_mppi', type=int, default=30,
                          help="Number of random samples in mppi.",
                          hierarchy=h + ["n_samples"])
        self.add_argument('--noise_factor_mppi', type=float, default=0.1,
                          help='The std factor of normal noise in mppi. Default 0.1.',
                          hierarchy=h + ["noise_factor"])  
        self.add_argument('--noise_decay_mppi', type=float, default=0.9,
                          help='Noise decay factor used in mppi.',
                          hierarchy=h + ["noise_decay"])
        self.add_argument('--weight_factor_mppi', type=float, default=0.001,
                          help='The weight factor of softmax in mppi. Default 0.001.',
                          hierarchy=h + ["weight_factor"])
        self.add_argument("--enable_reject_bad_mppi", action='store_true',
                          help='Enable rejecting bad updates in mppi.',
                          hierarchy=h + ["reject_bad"])
        self.add_argument('--no_early_stop_mppi', action='store_false',
                          dest='early_stop_mppi',
                          help="Enable/disable early stop when an adversarial example is found.",
                          hierarchy=h + ["early_stop"])
        h = ["find_feasible_solution", "cem_config"]
        self.add_argument('--n_steps_cem', type=int, default=8,
                          help="Steps of cem search.",
                          hierarchy=h + ["n_steps"])
        self.add_argument('--n_samples_cem', type=int, default=30,
                          help="Number of random samples in cem.",
                          hierarchy=h + ["n_samples"])
        self.add_argument('--jitter_factor_cem', type=float, default=1e-3,
                          help='The jitter factor in covariance matrix to make it positive definite in cem. Appropriate jitter can improve the performance. Default 1e-3.',
                          hierarchy=h + ["jitter_factor"])  
        self.add_argument('--elite_ratio_cem', type=float, default=0.01,
                          help='The ratio of elite samples among all samples in cem.',
                          hierarchy=h + ["elite_ratio"])
        self.add_argument('--min_n_elites_cem', type=int, default=10,
                          help='The minimum number of elite samples in cem. Default 10.',
                          hierarchy=h + ["min_n_elites"])
        self.add_argument("--enable_reject_bad_cem", action='store_true',
                          help='Enable rejecting bad updates in cem.',
                          hierarchy=h + ["reject_bad"])
        self.add_argument('--no_early_stop_cem', action='store_false',
                          dest='early_stop_cem',
                          help="Enable/disable early stop when an adversarial example is found.",
                          hierarchy=h + ["early_stop"])
        h = ["find_feasible_solution", "pgd_config"]
        self.add_argument('--n_steps_pgd', type=int, default=100,
                          help="Steps of PGD attack.",
                          hierarchy=h + ["n_steps"])
        self.add_argument('--n_restarts_pgd', type=int, default=30,
                          help="Number of random PGD restarts.",
                          hierarchy=h + ["n_restarts"])
        self.add_argument('--no_early_stop_pgd', action='store_false',
                          dest='early_stop',
                          help="Enable/disable early stop PGD when an adversarial example is found.",
                          hierarchy=h + ["early_stop"])
        self.add_argument('--lr_decay_pgd', type=float, default=0.99,
                          help='Learning rate decay factor used in PGD attack.',
                          hierarchy=h + ["lr_decay"])
        self.add_argument('--lr_decay_mode_pgd', type=str, default="auto",
                          help='Mode for Learning rate decay factor used in PGD attack. if auto, then determine decay factor based on num_steps and target factor.',
                          hierarchy=h + ["lr_decay_mode"])
        self.add_argument('--lr_target_pgd', type=float, default=0.05,
                          help='The target learning rate relative to the initial learning rate after decay.',
                          hierarchy=h + ["lr_target"])     
        self.add_argument('--restart_when_stuck_pgd', action='store_true',
                          help='Restart adversarial noise when they do not change over attack iterations.',
                          hierarchy=h + ["restart_when_stuck"])
        self.add_argument("--enable_reject_bad_pgd", action='store_true',
                          help='Enable rejecting bad updates in pgd.',
                          hierarchy=h + ["reject_bad"])
        h = ["debug"]
        ...
        self.add_argument("--sanity_check", action="store_true",
                          help="Using pgd upper bound as rhs-offset to check the sanity of the model.",
    ...
    def parse_config(self, args=None, verbose=True):
        ...
        if verbose and self["general"]["verbose"]:
