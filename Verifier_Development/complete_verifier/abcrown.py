...
from loading import load_model_and_vnnlib, parse_run_mode, adhoc_tuning, Customized, load_vnnlib  # pylint: disable=unused-import
...
class ABCROWN:
    ...
    def bab(self, data_lb, data_ub, c, rhs,
        ...
        if create_model or not hasattr(self, 'model'):
            self.model = LiRPANet(
        ...
        for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
            ...
            if enable_incomplete and not input_split:
                ...
                l, nodes, ret = self.bab(
                    ...
                    model=model_ori, create_model=refresh_model)
                ...
                l, nodes, ret = self.bab(
                    ...
                    vnnlib=vnnlib, model=model_ori, create_model=refresh_model)
    ...
    def main(self, interm_bounds=None):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            if run_mode != 'customized_data':
                if len(csv_item) == 3:
                    ...
                    # ANONYMOUS: use self.model if it is already loaded
                    if hasattr(self, 'model') and not refresh_model:
                        vnnlib, onnx_path = load_vnnlib(file_root, csv_item)
                        model_ori = self.model.model_ori
            ...
            if arguments.Config['attack']['pgd_order'] == 'before':
                ...
                if arguments.Config['debug']['sanity_check']:
                    ub_pgd = attack_margins.min().item()
                    print(f'Warning: change the RHS offset to the worst PGD upper bound {ub_pgd}.')
                    arguments.Config['specification']['rhs_offset'] = ub_pgd
                    rhs_offset = arguments.Config['specification']['rhs_offset']
                    vnnlib = add_rhs_offset(vnnlib, rhs_offset)
            else:
        ...
        self.logger.finish()
        # ANONYMOUS: reset the path
        if reset_path and run_mode == 'csv_file':
            arguments.Config['model']['onnx_path'] = None
            arguments.Config['specification']['vnnlib_path'] = None
        return self.logger.verification_summary

    # ANONYMOUS: add this function to init the class
    def init(self):
        torch.manual_seed(arguments.Config['general']['seed'])
        random.seed(arguments.Config['general']['seed'])
        np.random.seed(arguments.Config['general']['seed'])
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            print(f'\n {"%"*35} INIT on idx: {new_idx}, vnnlib ID: {vnnlib_id} {"%"*35}')
            # if arguments.Config['general']['save_output']:
            #     arguments.Globals['out']['idx'] = new_idx   # saved for test
            ...
            if run_mode != 'customized_data':
                ...
                if len(csv_item) == 3:
                    ...
                    model_ori, shape, vnnlib, onnx_path = load_model_and_vnnlib(
                        file_root, csv_item)
                    # arguments.Config['model']['input_shape'] = shape
                    # ANONYMOUS: if we init path here, the runmode will be changed in parse_run_mode()
                    # csv might be single file
                    # arguments.Config['model']['onnx_path'] = os.path.join(file_root, csv_item[0])
                    # arguments.Config['specification']['vnnlib_path'] = os.path.join(
                    #     file_root, csv_item[1])
                else:
            ...
            # ANONYMOUS: get speedup by init model to device
            model_ori = model_ori.to(device)
            x, data_max, data_min = x.to(device), data_max.to(device), data_min.to(device)
            # return
            c, rhs, pidx = batch_vnnlib(vnnlib)[0][1][0]
            c = torch.tensor(c,device=device,
                            dtype=torch.get_default_dtype())
            model_incomplete = cplex_processes = None
            self.model = LiRPANet(
                model_ori, c=c, cplex_processes=None,
                in_size=(data_min.shape if not len(pidx) > 1
                        else [len(pidx)] + list(data_min.shape[1:])))
            if arguments.Config['general']['verbose']:
                print_model(self.model.net)
            return


    # ANONYMOUS: add two functions to find and update constant node
    def find_constant_node_by_value(self, value):
        value = value.to(self.model.device, dtype=torch.get_default_dtype())
        assert hasattr(self, 'model')
        model = self.model.net
        node_list = []
        for node in model.nodes():
            if isinstance(node, BoundBuffers) and node.buffer.shape == value.shape and torch.allclose(node.buffer, value):
                if arguments.Config['general']['verbose']:
                    print(f'find node {node.name} has value {value}')
                node_list.append(node.name)
        return node_list

    def update_constant_node(self, node_list, new_value):
        new_value = new_value.to(self.model.device, dtype=torch.get_default_dtype())
        assert hasattr(self, 'model')
        model = self.model.net
        for node_name in node_list:
            node = model[node_name]
            assert isinstance(node, BoundBuffers) and node.buffer.shape == new_value.shape
            node.update_value(new_value)
            if arguments.Config['general']['verbose']:
                print(f'node {node_name} updated to {new_value}')
    
    # ANONYMOUS: update args to Config
