...
from auto_LiRPA.operators import BoundRelu, BoundLinear, BoundSqr, BoundAbs, BoundAdd
...
def default_pgd_loss(origin_out, output, C_mat, rhs_mat, cond_mat, same_number_const,
    ...
    if same_number_const:
        ...
        # ANONYMOUS: disable clamping temporarily
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat,
    ...
    '''
    search_func = arguments.Config["find_feasible_solution"]["search_func"]
    if search_func != "None":
        return eval(search_func)(model, X, data_min, data_max, C_mat, rhs_mat, cond_mat, 
                                              same_number_const, alpha, use_adam, normalize, initialization, 
                                              GAMA_loss, num_restarts, pgd_steps, only_replicate_restarts, 
                                              return_early_stopped, param_dict, **kwargs)
    ...
    reject_bad = arguments.Config["attack"]["reject_bad"] if reject_bad is None else reject_bad
    if arguments.Config["attack"]["pgd_lr_decay_mode"] == "auto":
        lr_decay = arguments.Config["attack"]["pgd_lr_target"] ** (1.0 / attack_iters)
    else:
    ...
    if prev_adv_example is not None:
        ...
        if delta.shape[2] == 1 and delta.shape[2] < adv_indices.shape[0]:
            ...
            replace_id = torch.randint(0, num_restarts,(num_examples,))
        ...
        # there are many domains and many adv examples, assume domains and examples are matched
            ...
            replace_id = torch.randint(0, num_restarts,(1,)).squeeze(-1)
    ...
    if use_adam:
        ...
        scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, lr_decay, verbose=True)
    ...
    for iteration in range(attack_iters + 1):
        ...
        if reject_bad:
            if iteration > 0:
                ...
                print(f"reject {bad_update_indices.sum().item()} bad updates from {num_restarts * num_or_spec} in iteration {i}")
        ...
        print(f"debug in iteration {iteration}")
        # print(f"loss: {loss.min().item()}, {loss.mean().item()}, {loss.max().item()}")
        # print(f"loss_gama: {loss_gama.item()}")
        print(f"output: {output.min().item()}, {output.mean().item()}, {output.max().item()}")
        # print(f"min idx: {output.argmin()}, input: {(delta+X)[0, output.argmin()]}")
...
class AdamClipping(Optimizer):
    ...
    @torch.no_grad()
    @torch.jit.script
    def _clip_update(exp_avg : torch.Tensor, denom : torch.Tensor, step_size : torch.Tensor, clipping_step_eps : torch.Tensor, lower_limit : torch.Tensor, upper_limit : torch.Tensor, p : torch.Tensor):
        ...
        d.clamp_(lower_limit, upper_limit)
        p.copy_(d)
