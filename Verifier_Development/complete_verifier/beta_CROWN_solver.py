...
class LiRPANet:
    ...
    # ANONYMOUS: add bound_upper, param_dict. param_dict is only used with crown-pgd
    def build(self, input_domain, x,
        ...
        if 'alpha-crown' in bounding_method:
            ...
            lb, ub, aux_reference_bounds = self.net.init_alpha(
                (x_expand,), share_alphas=share_alphas, c=self.c, bound_upper=bound_upper)
            ...
            ret = self.net.compute_bounds(
                ...
                bound_upper=bound_upper, aux_reference_bounds=aux_reference_bounds,
            ...
            ret = self.net.compute_bounds(
                x=(x_expand,), C=self.c, method='forward-optimized', bound_upper=bound_upper)
        else:
            with torch.no_grad():
                if bounding_method == 'init-crown':
                    ...
                    ret = self.net.compute_bounds(
                        ...
                        bound_upper=bound_upper, return_A=self.return_A,
                        needed_A_dict=self.needed_A_dict, param_dict=param_dict)
                    # ret_ibp = self.net.compute_bounds(
                    #     x=(x_expand,), C=self.c, method="IBP",
                    #     bound_upper=bound_upper, return_A=self.return_A,
                    #     needed_A_dict=self.needed_A_dict, param_dict=param_dict)
        ...
        lb, ub = self.get_interm_bounds(lb, ub)  # primals are better upper bounds
