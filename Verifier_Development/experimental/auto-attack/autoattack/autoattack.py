...
class AutoAttack():
    ...
    def run_standard_evaluation(self, x_orig, y_orig, bs=250, return_labels=False):
        ...
        with torch.no_grad():
            ...
            startt = time.time()
            margin_best = torch.ones([x_orig.shape[0]]).to(
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    logits = self.get_logits(adv_curr)
                    ...
                    margin_curr, _ = c.matmul(logits.unsqueeze(-1)).squeeze(-1).min(dim=1)

                    margin_smaller = margin_curr < margin_best[batch_datapoint_idcs]

                    false_batch = ~y.eq(output).to(robust_flags.device)
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
                    robust_flags[non_robust_lin_idcs] = False

                    x_adv[batch_datapoint_idcs[margin_smaller]] = adv_curr[margin_smaller].detach().to(x_adv.device)
                    y_adv[batch_datapoint_idcs[margin_smaller]] = output[margin_smaller].detach().to(x_adv.device)
                    margin_best[batch_datapoint_idcs[margin_smaller]] = margin_curr[margin_smaller]
