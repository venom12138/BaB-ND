...
class APGDAttack():
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None, best_margin=True):
        """
        :param x:           clean images
        :param y:           clean labels, if None we use the predicted labels
        :param best_loss:   if True the points attaining highest loss
                            are returned, otherwise adversarial examples
        :param best_margin: if True the points attaining smallest margin are returned, 
                            otherwise adversarial examples.
        """
        ...
        startt = time.time()
        if not best_loss and not best_margin:
            ...
            adv_best = x.detach().clone()
            margin_best = torch.ones([x.shape[0]]).to(
                self.device) * (float('inf'))
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _ = self.attack_single_run(x, y)
                margin_curr = self.model(best_curr)
                # create C matrix
                num_class = margin_curr.size(-1)
                c = (torch.eye(num_class).type_as(x)[y].unsqueeze(1) \
                    - torch.eye(num_class).type_as(x).unsqueeze(0))
                I = (~(y.unsqueeze(1) == torch.arange(num_class).type_as(y).unsqueeze(0)))
                c = (c[I].view(x.size(0), num_class - 1, num_class))  
                # import pdb; pdb.set_trace()
                margin_curr, _ = c.matmul(margin_curr.unsqueeze(-1)).squeeze(-1).min(dim=1)
                ind_curr = (margin_curr < margin_best).nonzero().squeeze()
                adv_best[ind_curr] = best_curr[ind_curr] + 0.
                margin_best[ind_curr] = margin_curr[ind_curr] + 0.
            return adv_best
                # if self.verbose:
                #     print('restart {} - loss: {:.5f}'.format(
                #         counter, loss_best.sum()))
...
class APGDAttack_targeted(APGDAttack):
    ...
    def perturb(self, x, y=None, x_init=None, best_margin=True):
        """
        :param x:           clean images
        :param y:           clean labels, if None we use the predicted labels
        """
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    adv[ind_to_fool[ind_curr]] = adv_curr[ind_curr].clone()
                    if best_margin:
                        margin_curr = self.model(adv_curr)
                        num_class = margin_curr.size(-1)
                        c = (torch.eye(num_class).type_as(x)[y_to_fool].unsqueeze(1) \
                            - torch.eye(num_class).type_as(x).unsqueeze(0))
                        I = (~(y_to_fool.unsqueeze(1) == torch.arange(num_class).type_as(y_to_fool).unsqueeze(0)))
                        c = (c[I].view(x_to_fool.size(0), num_class - 1, num_class))  
                        margin_curr, _ = c.matmul(margin_curr.unsqueeze(-1)).squeeze(-1).min(dim=1)

                        ind_curr = (margin_curr < margin_best[ind_to_fool]).nonzero().squeeze()
                        adv[ind_to_fool[ind_curr]] = adv_curr[ind_curr].clone()
                        margin_best[ind_to_fool[ind_curr]] = margin_curr[ind_curr] + 0.
