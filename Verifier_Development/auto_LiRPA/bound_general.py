...
class BoundedModule(nn.Module):
    ...
    def get_forward_value(self, node, store_fv_node_list=None):
        ...
        inputs = [self.get_forward_value(inp, store_fv_node_list) for inp in node.inputs]
        ...
        # ANONYMOUS: free forward_value for input nodes if they only have one output
        for inp in node.inputs:
            if len(inp.output_name) == 1 and inp not in self.roots() and \
                not isinstance(inp, BoundConstant) and hasattr(inp, 'forward_value') \
                and (store_fv_node_list is None or not (inp in store_fv_node_list or inp.name in store_fv_node_list)):
                delattr(inp, 'forward_value')
        # if store_fv_node_list is None or node in store_fv_node_list:
    ...
    def forward(self, *x, final_node_name=None, clear_forward_only=False,
            reset_perturbed_nodes=True, store_fv_node_list=None):
        ...
        if final_node_name:
            return self.get_forward_value(self[final_node_name], store_fv_node_list)
        else:
            return fill_template(
                deque([self.get_forward_value(self[n], store_fv_node_list)
    ...
    def check_prior_bounds(self, node, C=None, param_dict={}):
        ...
        if C is not None and isinstance(node, BoundConcat):
            ...
            for n in node.inputs:
                self.check_prior_bounds(n, param_dict=param_dict)
        tighten_input_bounds = (
            self.bound_opts['optimize_bound_args']['tighten_input_bounds']
        )
        directly_optimize_layer_names = (
        ...
        for i in range(len(node.inputs)):
            if (
                ...
                self.compute_intermediate_bounds(
                    node.inputs[i], prior_checked=True, param_dict=param_dict)
        node.prior_checked = True

    def compute_intermediate_bounds(self, node: Bound, prior_checked=False, param_dict={}):
        tighten_input_bounds = (
            self.bound_opts['optimize_bound_args']['tighten_input_bounds']
        )
        directly_optimize_layer_names = (
        ...
        if not prior_checked:
            self.check_prior_bounds(node, param_dict=param_dict)
        ...
        fast_pass = param_dict.get('fast_pass', False)
        disable_ref = param_dict.get('disable_ref', False)

        if not (fast_pass and node.name in reference_bounds):
            ...
            # For the first linear layer, IBP can give the same tightness as CROWN.
                ...
                if not skip:
                    if param_dict.get("unsound_method", None) != None and isinstance(self[node.output_name[0]], BoundRelu):
                        del newC
                        if param_dict["unsound_method"] == "mppi":
                            node.lower, node.upper = self.mppi_bounding(node, unstable_idx=unstable_idx, param_dict=param_dict)
                        elif param_dict["unsound_method"] == "pgd":
                            node.lower, node.upper = self.pgd_bounding(node=node, delete_bounds_after_use=True)
                        else: 
                            raise ValueError(f"unsound_method {param_dict['unsound_method']} is not supported")
                        ...
                        if self.return_A:
                            node.lower, node.upper, _ = self.backward_general(
                                ...
                                early_stop_conditioner = param_dict.get('early_stop_conditioner', None),
                            )
                        else:
                            ...
                            # ANONYMOUS: modify max_relu_layers
            ...
            node.lower, node.upper = reference_bounds[node.name]
        ...
        if node.name in reference_bounds:
            ...
            if not (fast_pass or disable_ref):
            ...
            # ANONYMOUS: add print
            num_domains = node.lower.shape[0]
            num_neurons = node.lower.shape[1]
            num_elements = num_domains * num_neurons
            num_lb_updates = torch.sum(ref_bounds[0] > node.lower).item()
            num_ub_updates = torch.sum(ref_bounds[1] < node.upper).item()
            # print(f"{num_lb_updates}/{num_elements} lower bounds are updated by reference. avg: {num_lb_updates/num_domains:.2f}/{num_neurons} neurons per domain")
            # print(f"{num_ub_updates}/{num_elements} upper bounds are updated by reference. avg: {num_ub_updates/num_domains:.2f}/{num_neurons} neurons per domain")
    ...
    def compute_bounds(
        ...
        elif method == 'crown-pgd':
            method = 'backward-pgd'
        elif method == 'alpha-crown-pgd':
            method = 'backward-pgd'
            compute_optimized = True
        elif method == 'crown-mppi':
            method = 'backward-mppi'
        elif method == 'alpha-crown-mppi':
            method = 'backward-mppi'
            compute_optimized = True

        if reference_bounds is None:
            reference_bounds = {}
            # ANONYMOUS: add a new attr outside crown
            if hasattr(self, 'default_reference_bounds'):
                reference_bounds = self.default_reference_bounds
    ...
    def _compute_bounds_main(self, C=None, method='backward', IBP=False,
            ...
            need_A_only=False, update_mask=None, param_dict={}):
        ...
        if param_dict == {} or param_dict == None:
            param_dict == {}
            param_dict["unsound_method"] = None
            assert ('pgd' not in method) and ('mppi' not in method), "unsound_method is specified but no params given"
        elif 'pgd' in method:
            param_dict["unsound_method"] = 'pgd'
        elif 'mppi' in method:
            param_dict["unsound_method"] = 'mppi'
        else:
            param_dict["unsound_method"] = None

        check_start_time = time.time()
        self.check_prior_bounds(final, C=C, param_dict=param_dict)
        check_end_time = time.time()
        print(f'Check prior bounds time: {check_end_time - check_start_time:.4f}s')
        if method == 'backward-pgd' or method == 'backward-mppi':
            method = 'backward'
        if method == 'backward':
            if param_dict.get("final_ibp", False):
                ret = self.IBP_general(node=final, C=C)
    ...
    def store_prev_bounds(self):
        lower_dict = {}
        upper_dict = {}
        interval_dict = {}
        for n in self.nodes():
            if getattr(n, 'lower', None) is not None:
                lower_dict[n.name] = n.lower
                upper_dict[n.name] = n.upper
                interval_dict[n.name] = n.interval
        self.lower_dict = lower_dict
        self.upper_dict = upper_dict
        self.interval_dict = interval_dict

    def restore_prev_bounds(self):
        lower_dict = self.lower_dict
        upper_dict = self.upper_dict
        interval_dict = self.interval_dict
        for n in self.nodes():
            if n.name in lower_dict:
                n.lower = lower_dict[n.name]
                n.upper = upper_dict[n.name]
                n.interval = interval_dict[n.name]

    def init_bounding_input(self, node, unstable_idx, param_dict, require_grad=False):
        total_restarts = param_dict['pgd_restarts']
        num_input = 0
        bound_input_list = []
        input_lb_list = []
        input_ub_list = []
        input_sample_list_for_lb = []
        input_sample_list_for_ub = []
        max_bound = 0
        output_shape = node.output_shape
        num_neuron = prod(output_shape[1:]) if unstable_idx is None else len(unstable_idx)
        for name, index in zip(self.input_name, self.input_index):
            if index is None:
                continue
            input_node = self[name]
            bound_input_list.append(input_node.value)
            if isinstance(input_node.value, (BoundedTensor, BoundedParameter)):
                ptb = input_node.value.ptb
            else:
                ptb = input_node.perturbation
            input_lb = ptb.x_L
            input_ub = ptb.x_U
            input_shape = input_lb.shape
            # batch should be constant for all inputs, and assume num_domain = batch
            num_domains = input_shape[0]
            restarts_per_domain_neuron = max(total_restarts //(num_domains * num_neuron), 1)
            num_repeat_domain = restarts_per_domain_neuron * num_neuron
            total_restarts = num_repeat_domain * num_domains
            input_lb = input_lb.repeat(num_repeat_domain, *[1]*(len(input_shape)-1))
            input_ub = input_ub.repeat(num_repeat_domain, *[1]*(len(input_shape)-1))
            # random generate input samples between input_lb and input_ub
            input_samples_for_lb = torch.rand((total_restarts, *input_shape[1:]), 
                                            device=self.device) * (input_ub - input_lb) + input_lb
            input_samples_for_ub = torch.rand((total_restarts, *input_shape[1:]),
                                            device=self.device) * (input_ub - input_lb) + input_lb
            if require_grad:
                input_samples_for_lb.requires_grad_(True)
                input_samples_for_ub.requires_grad_(True)
            max_bound = max(max_bound, (input_ub - input_lb).max().item())
            input_lb_list.append(input_lb)
            input_ub_list.append(input_ub)
            input_sample_list_for_lb.append(input_samples_for_lb)
            input_sample_list_for_ub.append(input_samples_for_ub)
            num_input += 1
            if num_input == self.num_global_inputs:
                break
        pre_activation_shape = [total_restarts, *output_shape[1:]]
        print(f"Bounding with PGD on {num_domains} input domains, {num_neuron} neurons, {total_restarts} restarts.")
        shape_dict = {"num_domains": num_domains, "num_neuron": num_neuron, "num_input": num_input,
                      "restarts_per_domain_neuron": restarts_per_domain_neuron, "num_repeat_domain": num_repeat_domain,
                      "total_restarts": total_restarts, "pre_activation_shape": pre_activation_shape}
        input_dict = {"input_sample_list_for_lb": input_sample_list_for_lb, "input_sample_list_for_ub": input_sample_list_for_ub,
                      "input_lb_list": input_lb_list, "input_ub_list": input_ub_list, "bound_input_list": bound_input_list,
                      "max_bound": max_bound}
        return shape_dict, input_dict

    def perform_pgd(self, node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=False, reject_bad=True):
        pgd_steps = param_dict['pgd_steps']
        lr_decay = param_dict['lr_decay']
        auto_factor = param_dict['auto_factor']
        num_domains, num_neuron, num_input, restarts_per_domain_neuron, num_repeat_domain, total_restarts, pre_activation_shape = \
            shape_dict["num_domains"], shape_dict["num_neuron"], shape_dict["num_input"], shape_dict["restarts_per_domain_neuron"], \
            shape_dict["num_repeat_domain"], shape_dict["total_restarts"], shape_dict["pre_activation_shape"]
        input_lb_list, input_ub_list, bound_input_list, max_bound = input_dict["input_lb_list"], input_dict["input_ub_list"], input_dict["bound_input_list"], input_dict["max_bound"]
        if bound_ub:
            input_sample_list = input_dict["input_sample_list_for_ub"]
        else:
            input_sample_list = input_dict["input_sample_list_for_lb"]
        
        alpha = max_bound / auto_factor
        optimizer = torch.optim.Adam(input_sample_list, lr=alpha)
        scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, lr_decay, verbose=False)

        def loss_fn(pre_activation):
            # pre_activation: (total_restarts, num_neuron), total_restarts = num_domains * restarts_per_domain_neuron * num_neuron 
            loss_pre = torch.diagonal(pre_activation.view(num_domains * restarts_per_domain_neuron, num_neuron, num_neuron), dim1=1, dim2=2)
            return loss_pre, loss_pre.mean()
        for i in range(pgd_steps + 1):
            # print memory allocation
            print(f"Mem allocated in step{i}: {torch.cuda.memory_allocated() / 1024 / 1024}")
            # optimizer.zero_grad()
            if not reject_bad and i == pgd_steps:
                break
            pre_activation = self.forward(input_sample_list, final_node_name=node.name).view(total_restarts, -1)
            self._clear_and_set_new(None)
            if unstable_idx is not None:
                pre_activation = pre_activation[:, unstable_idx]
            loss_pre, loss = loss_fn(pre_activation)
            if bound_ub:
                loss_pre = -loss_pre 
                loss = -loss
            if reject_bad:
                # if large step size increase loss, reject the step
                if i > 0:
                    # num_domains * restarts_per_domain_neuron, num_neuron
                    bad_update_indices = (loss_pre > loss_before)
                    print(f"reject {bad_update_indices.sum().item()} bad updates from {total_restarts} in iteration {i}")
                    loss_pre[bad_update_indices] = loss_before[bad_update_indices]
                    bad_update_indices = bad_update_indices.view(-1)
                    pre_activation.data[bad_update_indices] = pre_activation_before[bad_update_indices]
                    for j in range(num_input):
                        input_sample_list[j].data[bad_update_indices] = input_sample_list_before[j][bad_update_indices]
                pre_activation_before = pre_activation.clone()
                input_sample_list_before = [input_sample.clone() for input_sample in input_sample_list]
                loss_before = loss_pre.clone()
                if i == pgd_steps:
                    break
            loss.backward()
            optimizer.step()
            ...
            for j in range(num_input):
                input_sample_list[j].data.clamp_(input_lb_list[j], input_ub_list[j])
        pre_activation = pre_activation.detach()
        if bound_ub:
            if unstable_idx is None:
                est_bounds = torch.max(pre_activation.view(num_repeat_domain, num_domains, *pre_activation_shape[1:]), dim=0)[0]
            else:
                est_bounds = torch.max(pre_activation.view(num_repeat_domain, num_domains, num_neuron), dim=0)[0]
        else:
            if unstable_idx is None:
                est_bounds = torch.min(pre_activation.view(num_repeat_domain, num_domains, *pre_activation_shape[1:]), dim=0)[0]
            else:
                est_bounds = torch.min(pre_activation.view(num_repeat_domain, num_domains, num_neuron), dim=0)[0]

        del optimizer, scheduler
        del loss_pre, loss, pre_activation
        if reject_bad:
            for j in range(num_input):
                del input_sample_list_before[j]
            del pre_activation_before, input_sample_list_before, loss_before, bad_update_indices
        return est_bounds

    def pgd_bounding(self, node, unstable_idx, param_dict):
        time_start = time.time()
        self.store_prev_bounds()
        prev_grad_enabled = torch.is_grad_enabled()
        if not prev_grad_enabled:
            torch.torch.set_grad_enabled(True)
        assert param_dict is not None
        reject_bad = True
        shape_dict, input_dict = self.init_bounding_input(node, unstable_idx, param_dict, require_grad=True)
        num_input = shape_dict["num_input"]
        input_sample_list_for_lb, input_sample_list_for_ub, input_lb_list, input_ub_list, bound_input_list, max_bound = \
            input_dict["input_sample_list_for_lb"], input_dict["input_sample_list_for_ub"], input_dict["input_lb_list"], \
            input_dict["input_ub_list"], input_dict["bound_input_list"], input_dict["max_bound"]
        
        lower_bounds = self.perform_pgd(node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=False, reject_bad=reject_bad)
        upper_bounds = self.perform_pgd(node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=True, reject_bad=reject_bad)
        if not prev_grad_enabled:
            torch.torch.set_grad_enabled(False)
        # reset model
        with torch.no_grad():
            self.forward(bound_input_list)
        self.restore_prev_bounds()
        for j in range(num_input):
            del input_sample_list_for_lb[j], input_sample_list_for_ub[j], input_lb_list[j], input_ub_list[j]
        del input_sample_list_for_lb, input_sample_list_for_ub, input_lb_list, input_ub_list

        print('PGD bounding time: ', time.time() - time_start)
        return lower_bounds, upper_bounds

    def sample_input(self, input_lb_list, input_ub_list, restarts_per_domain_neuron, init_input_list, eps):
        input_sample_list = []
        device = self.device

        for i, init_input in enumerate(init_input_list):
            # init_input: (num_domains, num_neuron, input_shape)
            num_domains, num_neuron = init_input.shape[0:2]
            input_shape = init_input.shape[2:]
            new_input = init_input.unsqueeze(1).repeat(1, restarts_per_domain_neuron, 1, *[1]*len(input_shape)) \
                                                .view(num_domains * restarts_per_domain_neuron * num_neuron, *input_shape)
            noise = torch.normal(mean=0, std=eps, size=new_input.shape, device=device) \
                                    * (input_ub_list[i] - input_lb_list[i]) / 2
            input_sample_list.append((new_input + noise).data.clamp_(input_lb_list[i].unsqueeze(0), input_ub_list[i].unsqueeze(0)))
        return input_sample_list

    def perform_mppi(self, node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=False, reject_bad=True):
        pgd_steps = param_dict['pgd_steps']
        num_domains, num_neuron, num_input, restarts_per_domain_neuron, num_repeat_domain, total_restarts, pre_activation_shape = \
            shape_dict["num_domains"], shape_dict["num_neuron"], shape_dict["num_input"], shape_dict["restarts_per_domain_neuron"], \
            shape_dict["num_repeat_domain"], shape_dict["total_restarts"], shape_dict["pre_activation_shape"]
        input_lb_list, input_ub_list = input_dict["input_lb_list"], input_dict["input_ub_list"]
        weight_factor = param_dict['mppi_weight_factor']
        init_input_list = []
        for input_lb, input_ub in zip(input_lb_list, input_ub_list):
            # input_lb, input_ub: total_restarts, input_shape
            init_input = (input_lb[0:num_domains] + input_ub[0:num_domains])/2
            input_shape = init_input.shape[1:]
            init_input_list.append(init_input.unsqueeze(1).repeat(1, num_neuron, *[1]*len(input_shape)))
        # reject_bad = False
        def loss_fn(pre_activation):
            # pre_activation: (total_restarts, num_neuron), total_restarts = num_domains * restarts_per_domain_neuron * num_neuron 
            loss_pre = torch.diagonal(pre_activation.view(num_domains * restarts_per_domain_neuron, num_neuron, num_neuron), dim1=1, dim2=2)
        ...
        with torch.no_grad():
            for i in range(pgd_steps + 1):
                ...
                if not reject_bad and i == pgd_steps:
                    break
                input_sample_list = self.sample_input(input_lb_list, input_ub_list, restarts_per_domain_neuron, init_input_list, param_dict['mppi_noise_factor'])
                pre_activation = self.forward(input_sample_list, final_node_name=node.name).view(total_restarts, -1)
                self._clear_and_set_new(None)
                if unstable_idx is not None:
                    pre_activation = pre_activation[:, unstable_idx]
                ...
                if reject_bad:
                    ...
                    input_sample_list_before = [input_sample.clone() for input_sample in input_sample_list]
                    loss_before = loss_pre.clone()
                # larger loss, smaller weight
                # so if bound lb, we should negate the loss before
                # num_domains, restarts_per_domain_neuron, num_neuron
                scores = -(loss_pre.view(num_domains, restarts_per_domain_neuron, num_neuron))
                scores_min = scores.min(dim=1, keepdim=True)[0]
                scores_max = scores.max(dim=1, keepdim=True)[0]
                scores = (scores - scores_min) / (scores_max - scores_min).clamp(min=1e-6)
                # if scores.numel() < 2:
                ...
                init_input_list = []
                for j in range(num_input):
                    # total_restarts, input_shape
                    input_shape = input_lb_list[j].shape[1:]
                    # num_domains, num_neuron, input_shape
                    init_input = torch.sum(weights.view(*weights.shape, *[1]*len(input_shape)) * input_sample_list[j]
                                           .view(num_domains, restarts_per_domain_neuron, num_neuron, *input_shape), dim=1, keepdim=False)
                    init_input.data.clamp_(input_lb_list[j][0:num_domains].unsqueeze(1), input_ub_list[j][0:num_domains].unsqueeze(1))
                    init_input_list.append(init_input)
    ...
    def mppi_bounding(self, node, unstable_idx, param_dict):
        time_start = time.time()
        self.store_prev_bounds()
        prev_grad_enabled = torch.is_grad_enabled()
        ...
        assert param_dict is not None
        reject_bad = True
        shape_dict, input_dict = self.init_bounding_input(node, unstable_idx, param_dict)
        num_input = shape_dict["num_input"]
        input_lb_list, input_ub_list, bound_input_list = input_dict["input_lb_list"], input_dict["input_ub_list"], input_dict["bound_input_list"]
        
        lower_bounds = self.perform_mppi(node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=False, reject_bad=reject_bad)
        upper_bounds = self.perform_mppi(node, unstable_idx, param_dict, shape_dict, input_dict, bound_ub=True, reject_bad=reject_bad)
        if prev_grad_enabled:
            torch.torch.set_grad_enabled(True)
        # reset model
        with torch.no_grad():
        ...
        self.restore_prev_bounds()
        for j in range(num_input):
            del input_lb_list[j], input_ub_list[j]
        del input_lb_list, input_ub_list

        print('PGD bounding time: ', time.time() - time_start)
        print(f"Mean lower bound: {lower_bounds.mean().item()}, mean upper bound: {upper_bounds.mean().item()}")
        return lower_bounds, upper_bounds
