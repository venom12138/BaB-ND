...
def backward_general(
    ...
    if early_stop_conditioner is not None:
        early_stop_conditioner.clear()
    while len(queue) > 0:
        l = queue.popleft()  # backward from l
        self.backward_from[l.name].append(bound_node)
        if early_stop_conditioner is not None:
            early_stop_nodes = early_stop_conditioner.update(l)
        ...
        if l in early_stop_nodes:
            early_stop_conditioner.process(l, self)
            continue
...
def concretize_root(root, batch_size, output_dim,
    ...
    if root in extra_nodes:
        perturbation = PerturbationLpNorm(x_L=root.lower, x_U=root.upper)
        input_shape = root.lower.shape
        # concretized_lb = perturbation.concretize(
        #     (perturbation.x_L + perturbation.x_U) / 2, lA, 
        #     sign=-1) if (lA is not None) else None
        # concretized_ub = perturbation.concretize(
        #     (perturbation.x_L + perturbation.x_U) / 2, uA, 
        #     sign=+1) if (uA is not None) else None
        # lb = lb + concretized_lb if (lA is not None) else None
        # ub = ub + concretized_ub if (uA is not None) else None
        lb = perturbation.concretize(
            (perturbation.x_L + perturbation.x_U) / 2, lA, 
            sign=-1) if (lA is not None) else None
        ub = perturbation.concretize(
            (perturbation.x_L + perturbation.x_U) / 2, uA, 
            sign=+1) if (uA is not None) else None
    elif hasattr(root, 'perturbation') and root.perturbation is not None:
...
def concretize(self, batch_size, output_dim, lb=None, ub=None,
               ...
               extra_nodes=[]):
    roots = self.roots()
    # ANONYMOUS: add extra_nodes to the roots
    if len(extra_nodes) > 0:
        for node in extra_nodes:
            assert node not in roots
        # # if we want to concretize on extra nodes before roots, we should remove perturbated input nodes
        # for node in roots:
        #     if isinstance(node, BoundInput) and node.perturbed:
        #         roots.remove(node)
        roots = extra_nodes + roots
    ...
    for i in range(len(roots)):
        ...
        roots[i].lb, roots[i].ub = concretize_root(
            ...
            node_start=node_start, input_shape=roots[0].center.shape if hasattr(roots[0], 'center') else None,
