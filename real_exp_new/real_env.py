...
from real_utils import depth2fgpcd, rpy_to_rotation_matrix


def decode_action_1(action, state):
    mean_x = state[:, 0].mean().item()
    mean_z = state[:, 2].mean().item()
    push_length = 0.10
    r = action[0]
    theta = action[1]
    phi = action[2]
    action_repeat = int(action[3])
    x_start = r * np.cos(theta) + mean_x
    z_start = r * np.sin(theta) + mean_z
    x_end = x_start - push_length * action_repeat * np.cos(theta + phi)
    z_end = z_start - push_length * action_repeat * np.sin(theta + phi)
    return x_start, z_start, x_end, z_end


def decode_action_2(action):
    push_length = 0.10
    x_start = action[0]
    z_start = action[1]
    theta = action[2]
...
class RealEnv:
    def __init__(self, 
        ...
        self.WRIST = '311322300308'  # device id of the wrist camera
        num_cams = num_fixed_cams + (1 if use_wrist_cam else 0)
        self.serial_numbers = SingleRealsense.get_connected_devices_serial()
        print(f"Connected devices: {self.serial_numbers}")
        assert len(self.serial_numbers) == num_cams, f"Expected {num_cams} cameras, got {len(self.serial_numbers)}"
        if use_wrist_cam:
        ...
        self.calibrate_result_dir = f'{file_dir}/calibration_result'
        ...
        self.bbox = np.array([[-0.5, 0.5], [-0.3, 0.7], [-0.2, 0.1]])  # bounding box of the workspace
    ...
    def fixed_camera_calibrate(self, visualize=True, save=True, return_results=True):
        ...
        if save:
            ...
            with open(f'{self.calibrate_result_dir}/tvecs.pkl', 'wb') as f:
                pickle.dump(tvecs, f)
            with open(f'{self.calibrate_result_dir}/intrinsics.pkl', 'wb') as f:
                pickle.dump(intr_vecs, f)
        if return_results:
            return rvecs, tvecs
