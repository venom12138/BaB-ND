...
class SingleRealsense(mp.Process):
    ...
    def __init__(
        ...
        if visible_area is not None:
            shape = (int(shape[0] * (visible_area[1][1] - visible_area[1][0])), 
                     int(shape[1] * (visible_area[0][1] - visible_area[0][0])))
        examples = dict()
        if enable_color:
        ...
        if enable_depth:
            ...
            if extrinsics_dir is None:
                ...
                warnings.warn('extrinsics_dir is None, using identity matrix.') # no worries
            else:
        ...
        if enable_pcd:
            examples['points'] = np.empty(
                shape=shape+(3,), dtype=np.float32)
        ...
        self.visible_area = [[round(visible_area[1][0] * resolution[1]), round(visible_area[1][0] * resolution[1]) + shape[0]],
                             [round(visible_area[0][0] * resolution[0]), round(visible_area[0][0] * resolution[0]) + shape[1]]] if visible_area is not None else None
        # self.visible_area = np.array(self.visible_area).astype(int) if visible_area is not None else None
        ...
        # if open this process
        self.already_start = 0
            ...
            while not self.stop_event.is_set():
                ...
                if self.enable_color:
                    ...
                    if self.visible_area is not None:
                        data['color'] = np.asarray(color_frame.get_data())[self.visible_area[0][0]:self.visible_area[0][1], self.visible_area[1][0]:self.visible_area[1][1]]
                    else:
                ...
                if self.enable_depth:
                    ...
                    if self.visible_area is not None:
                        data['depth'] = np.asarray(proc_depth_frame.get_data())[self.visible_area[0][0]:self.visible_area[0][1], self.visible_area[1][0]:self.visible_area[1][1]]
                    else:
                ...
                if self.enable_infrared:
                    if self.visible_area is not None:
                        data['infrared'] = np.asarray(
                            frameset.get_infrared_frame().get_data())[self.visible_area[0][0]:self.visible_area[0][1], self.visible_area[1][0]:self.visible_area[1][1]]
                    else:
                        data['infrared'] = np.asarray(
                ...
                if self.enable_pcd:
                    pc = rs.pointcloud()
                    if self.visible_area is not None:
                        data['points'] = np.asarray(
                            pc.calculate(proc_depth_frame).get_vertices()).view(np.float32).reshape(h, w, 3)[self.visible_area[0][0]:self.visible_area[0][1], self.visible_area[1][0]:self.visible_area[1][1]]
                    else:
                        data['points'] = np.asarray(
                            pc.calculate(proc_depth_frame).get_vertices()).view(np.float32).reshape(h, w, 3)
