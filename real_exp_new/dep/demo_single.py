...
class ImageProcessor(mp.Process):
    ...
    def __init__(self, serial_number, realsense, capture_fps, record_fps, record_time, record_flag):
        super().__init__()
        # self.index = index
        ...
        mesh_pcd, R, T, rotation_matrices, split_num = get_params(serial_number)
    ...
    def run(self):
        ...
        while self.alive:
            try:
                ...
                if out['step_idx'] == next_step_idx * (capture_fps // record_fps):
                    if next_step_idx == 0:
                        start_time = time.time()
                    print(f'step {out["step_idx"]} step {out["step_idx"]} timestamp {out["timestamp"]}')
                    timestamp = out['timestamp']
                    process(out['color'], out['depth'], out['points'], self.mesh_pcd, self.R, self.T, self.rotation_matrices, self.split_num)
                    # with self.lock:
...
def process(colors, depth, points, mesh_pcd, R, T, rotation_matrices, split_num):
    verbose = True
    save_img = False
    FLOAT_TYPE = np.float16
    if save_img:
        cv2.imwrite(f'{file_dir}/test.png', colors)
        print(f"save image to {file_dir}/test.png")
    process_start_time = time.time()
    colors = (colors/255).astype(FLOAT_TYPE).reshape(-1,3)
    # depth = (depth/1000).astype(FLOAT_TYPE).flatten()
    points = (points).astype(FLOAT_TYPE).reshape(-1, 3)
    ...
    color_mask = np.linalg.norm(valid_colors - block_color, axis=1, ord=2) < 0.5
    object_points = valid_points[color_mask]
    assert len(object_points) > 0
    l, u = np.min(object_points[:, 2]), np.max(object_points[:, 2])
    # z_ratio = 0.7
    # z_mask = np.logical_and(object_points[:, 2] >= l, object_points[:, 2] <= l + (u - l) * z_ratio)
    # object_points = object_points[z_mask]
    object_points[:, 2] = l
    ...
    o3d.visualization.draw_geometries([pcd, object_pcd, mesh_pcd, coordinate])

    return

def get_params(serial_number = '246322301893'):
    FLOAT_TYPE = np.float16
    ...
    R = cam2base[:3, :3].T.astype(FLOAT_TYPE)
    T = cam2base[:3, 3].astype(FLOAT_TYPE)
    return mesh_pcd, R, T, rotation_matrices, split_num

def main():
    serial_number = '246322301893'
    # # visible_area: w, h
    # if serial_number == '246322301893':
    #     visible_area = [[0.15, 0.7], [0.1, 0.65]]
    # elif serial_number == '246322303954':
    #     visible_area = [[0.2, 0.7], [0.1, 0.65]]
    # elif serial_number == '311322303615':
    #     visible_area = [[0.25, 0.75], [0.0, 0.70]]
    # elif serial_number == '311322300308':
    #     visible_area = [[0.30, 0.80], [0.3, 1]]
    # else:
    #     visible_area = None
    # 311322303615, 215122252880
    if serial_number == '246322301893':
    ...
    try:
        ...
        process = ImageProcessor(serial_number, camera, record_fps, record_fps, record_time, record_flag)
        process.start()
