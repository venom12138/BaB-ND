...
def nms(boxes, scores, threshold, obj_num, size_similarity_threshold=0.8):
    if len(boxes) == 0:
        return []
    ...
    boxes_converted[:, 0] = boxes[:, 0] - boxes[:, 2] / 2  # x1
    boxes_converted[:, 1] = boxes[:, 1] - boxes[:, 3] / 2  # y1
    boxes_converted[:, 2] = boxes[:, 0] + boxes[:, 2] / 2  # x2
    boxes_converted[:, 3] = boxes[:, 1] + boxes[:, 3] / 2  # y2
    boxes = boxes_converted

    # Compute area of each box and filter based on size similarity
    areas = (boxes[:, 2] - boxes[:, 0]) * (boxes[:, 3] - boxes[:, 1])
    valid_indices = np.arange(len(areas))
    if len(areas) > 4:
        median_area = np.sort(areas)[min(2, len(areas) - 1)]
        min_area = median_area * (1 - size_similarity_threshold)
        max_area = median_area * (1 + size_similarity_threshold)
        valid_indices = np.where((areas >= min_area) & (areas <= max_area))[0]

    # Filter boxes and scores based on valid_indices
    ...
    while order.size > 0 and len(keep) < obj_num:
        i = order[0]
        keep.append(valid_indices[i])  # Map back to original index
    ...
    # Ensure that we only keep obj_num boxes
    return keep[:obj_num]
