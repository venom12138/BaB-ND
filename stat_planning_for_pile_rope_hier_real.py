...
# task specific
import planning.planning_for_pile_hier_real_sync as planning_for_pile
...
def main(config: DictConfig):
    ...
    if task_name == "obj_pile":
        ...
        import planning.planning_for_rope_hier_real_sync as planning_for_rope
    ...
    abcrown_verbose = (
        planning_config["abcrown_verbose"] and ("CROWN" in method_types or "CROWN" in force_method_types)
    )
    ...
    # state_dict = torch.load(model_file) # .eval()
    # model = eval(config["model"])(config).to(config.planning.device)
    ...
    for i in range(num_test):
        ...
        if not all_reload and not config["planning"]["gather_data"]:
            if task_name == "obj_pile":
                for method_type in method_types:
                    # plot long horizon
                    state_gt = np.array(all_res[model_name][i][method_type]['result_summary']['init_res']["result_summary"]['gt_states']) # seq, state_dim (not scaled)
                    action_seq = state_gt[1:, -2:] # seq-1, action_dim (not scaled)
                    pusher_poses = np.concatenate([state_gt[1:, -4:-2], [state_gt[-1, -4:-2] + action_seq[-1]]])
                    state_gt = state_gt[:, :config["data"]["state_dim"]]
                    labels = np.array(all_res[model_name][i][method_type]['result_summary']['labels'])
                    plot_obj_pile(np.expand_dims(state_gt, axis=0), np.expand_dims(state_gt, axis=0), \
                            np.expand_dims(pusher_poses, axis=0), np.expand_dims(action_seq, axis=0), \
                            config, int(i), save_path='/'.join(json_file_path.split('/')[:-1]), labels=labels, \
                            filename=f"{num_test}_{i}_cmp_{method_type}_{all_res[model_name][i][method_type]['result_summary']['final_cost']:.3f}_long")
                    # plot execution
                    ...
                    plot_cost(pred_cost_seq, gt_cost_seq, cost_diff_seq, \
                            save_path='/'.join(json_file_path.split('/')[:-1]), \
                ...
                plot_cost(pred_cost_seq, gt_cost_seq, cost_diff_seq, \
                        save_path='/'.join(json_file_path.split('/')[:-1]), \
                        filename=f"{config['planning']['num_test']}_{i}_cost_{method_type}_{round(all_res[model_name][i][method_type]['result_summary']['final_cost'], 3)}.png")
            else:
    ...
    #     os.system(f"rm -rf {crown_name_dict['crown_config_prefix']}{crown_name_dict['vnnlib_name']}")
    #     os.system(f"rm -rf {crown_name_dict['crown_config_prefix']}{crown_name_dict['vnnlib_name']}.compiled")
    #     os.system(f"rm -rf {crown_name_dict['warmstart_file']}")
    #     os.system(f"rm -rf {crown_name_dict['crown_sol_file']}")
    #     exit(1)
