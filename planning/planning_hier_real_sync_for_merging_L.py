...
def planning(
    ...
    pusher_pos_lb, pusher_pos_ub = -window_size * 0.05, window_size * 1.05
    ...
    enable_visualize = False # True # False # True #  
    if enable_real:
        ...
        # input_key = input("Press x to add a breakpoint")
        # if input_key == 'x':
        #     while True:
        #         env.get_state(
        #             keypoint_offsets_2d,
    ...
    if method_type == "CROWN":
        ...
        if abcrown_verbose or False:
            analyze_search_progress(bab_file, vis_file_name_prefix)
    ...
    max_horizon = max(max_horizon, math.ceil(subgoal_interval * long_action_bound / action_bound)+10)
    ...
    planner_mppi.noise_level = action_bound * 0.75 if hier_config.get("noise_level", 'auto') else hier_config["noise_level"],
    planner_mppi.noise_type = 'normal'
    planner_mppi.evaluate_traj = partial(reward_local, reward_config=reward_config)
    ...
    debug_figs = []
    follow_pusher_replan = False
    last_mppi_init_act_seq_for_replan = None
    adaptive_horizon = None
    while exec_time < time_limit:
        ...
        if i > 0:
            ...
            # reach the subgoal
            if ((not follow_pusher) and 
                (((subgoal_cost < reach_threshold or subgoal_step >= max_horizon) and not follow_pusher_replan) or 
                ((subgoal_cost < reach_threshold or subgoal_step >= max_horizon+12) and follow_pusher_replan))) \
                or (follow_pusher and reach_pusher_pos(curr_pusher_pos, pusher_pos_sequnce_sub[subgoal_id])):
                if (not follow_pusher) and (subgoal_step >= max_horizon) and subgoal_id < num_subgoal-1 and subgoal_cost >= reach_threshold: # TODO: need more test to determine num_subgoal-1 or num_subgoal
                ...
                while (follow_pusher and reach_pusher_pos(curr_pusher_pos, pusher_pos_sequnce_sub[subgoal_id])):
                    subgoal_id, subgoal_state, follow_pusher, inner_idx = update_subgoal(subgoal_id)
                subgoal_step = 0
                follow_pusher_replan = False
                if follow_pusher and (step_cost(env_state[:state_dim//2] / state_scale, subgoal_state[:state_dim//2], cost_norm) > 0.7 \
                    or step_cost(env_state[state_dim//2:state_dim] / state_scale, subgoal_state[state_dim//2:state_dim], cost_norm) > 0.7):
                    follow_pusher_replan = True
                    follow_pusher = False
                    print("Do not follow pusher since Far from the subgoal.")

            # reach the target state or the last subgoal
            if actual_step_cost < 1.0 or (buffer_round == 0 and subgoal_id == num_subgoal):
                success = True
        
        # import pdb; pdb.set_trace()

        print(f"\n\n{'-'*5}step {i}{'-'*5}  Current subgoal: {subgoal_id}, follow_pusher: {follow_pusher}.\n")
        print(f"    Current pusher pos: {curr_pusher_pos}.")
        print(f"Subgoal state: {subgoal_state}.")
        ...
        if follow_pusher and reach_pusher_pos(curr_pusher_pos, inner_pusher_pos_seq[inner_idx]):
            inner_idx += 1
        if follow_pusher:
            print(f"inner_idx: {inner_idx}, inner_pusher_pos_seq: {inner_pusher_pos_seq[inner_idx]}")
        ...
        # if horizon > 10 and not follow_pusher:
        #     import pdb; pdb.set_trace()
        ...
        if success:
            ...
            mppi_init_act_seq = torch.zeros((horizon, action_dim), device=device)
            planner_mppi.n_update_iter = hier_config["n_update_iter"]
        elif follow_pusher_replan and subgoal_id < num_subgoal:
            assert follow_pusher is False
            horizon = 20 if adaptive_horizon is None else adaptive_horizon
            if last_mppi_init_act_seq_for_replan is not None:
                mppi_init_act_seq = torch.zeros((horizon, action_dim), device=device)
                mppi_init_act_seq[1:] = last_mppi_init_act_seq_for_replan[1:len(mppi_init_act_seq)]
            else:
                mppi_init_act_seq = torch.zeros((horizon, action_dim), device=device)
            reward_config["only_final_cost"] = False # True
            planner_mppi.noise_level = 0.125
            planner_mppi.action_lower_lim = action_lb
            planner_mppi.action_upper_lim = action_ub
            planner_mppi.n_update_iter = 15
        elif follow_pusher:
            horizon = 1
            ...
            horizon = min(horizon + 5, max(horizon, 10))
            mppi_init_act_seq = torch.zeros((horizon, action_dim), device=device)
        ...
        print(f"{method_type} time: {time.time()-sampling_start_time}")
        if follow_pusher_replan:
            last_mppi_init_act_seq_for_replan = res["act_seq"]
            adaptive_horizon = max(horizon - 1, 7)
        else:
            last_mppi_init_act_seq_for_replan = None
            adaptive_horizon = None
        ...
        print(f"expected pusher pos: {next_pusher_pos}; real pusher pos: {np.array(env.get_pusher_position())[:2]*1000}")
        ...
        if enable_real:
            ...
            axes[0,0].set_title(f"Plan:fp_replan:{int(follow_pusher_replan)} horizon:{horizon}")
            axes[0,1].set_title(f"After pushing:fp:{int(follow_pusher)}")
            axes[0,2].set_title(f"Subgoals:id:{subgoal_id}")
            
            assert task_name == "merging_L"
            obj_type = "L"
            num_kp = state_dim // 2 // 2

            # plot target state
            draw_object_and_pusher(ax=axes[0,0], \
                                object_type=obj_type, \
                                keypoints=state_scale*scaled_target_state[:num_kp * 2].reshape(num_kp,2), \
            ...
            draw_object_and_pusher(ax=axes[0,0], \
                                object_type=obj_type, \
                                keypoints=state_scale*scaled_target_state[num_kp * 2: state_dim].reshape(num_kp,2), \
                                # pusher=env_state[state_dim:state_dim+2], \
                                color='blue', \
                                alpha=1.0
                                )
            
            draw_object_and_pusher(ax=axes[0,1], object_type=obj_type, \
                                keypoints=state_scale*scaled_target_state[:num_kp * 2].reshape(num_kp,2), \
                                pusher=env_state[state_dim:state_dim+2], \
                                color='r', \
                                alpha=1.0
            ...
            draw_object_and_pusher(ax=axes[0,2], object_type=obj_type, \
                                keypoints=state_scale*scaled_target_state[:num_kp * 2].reshape(num_kp,2), \
                                pusher=env_state[state_dim:state_dim+2], \
                                color='r', \
                                alpha=1.0
            ...
            draw_object_and_pusher(ax=axes[0,0], object_type=obj_type, \
                                keypoints=env_state[:num_kp * 2].reshape(num_kp,2), \
                                pusher=env_state[state_dim:state_dim+2], \
                                color='orange', \
                                alpha=1.0,
                                )
            draw_object_and_pusher(ax=axes[0,0], object_type=obj_type, \
                                keypoints=env_state[num_kp * 2: state_dim].reshape(num_kp,2), \
                                # pusher=env_state[state_dim:state_dim+2], \
                                color='cyan', \
                                alpha=1.0,
                                )
            ...
            for k, subgoal_state_for_debug in enumerate(state_sequence_sub):
                if k == subgoal_id:
                    alpha = 1.0
                    c_for_subgoal = ['darkred', 'navy']
                else:
                    alpha = 0.8 - 0.6 / len(state_sequence_sub) * k
                    c_for_subgoal = ['pink', 'cornflowerblue']
                draw_object_and_pusher(ax=axes[0,2], object_type=obj_type, \
                                keypoints=state_scale*subgoal_state_for_debug[:num_kp * 2].reshape(-1,2), \
                                pusher=env_state[state_dim:state_dim+2], \
                                color=c_for_subgoal[0], \
                                alpha=alpha,
                                )
                draw_object_and_pusher(ax=axes[0,2], object_type=obj_type, \
                                keypoints=state_scale*subgoal_state_for_debug[num_kp * 2: state_dim].reshape(-1,2), \
                                # pusher=env_state[state_dim:state_dim+2], \
                                color=c_for_subgoal[1], \
                                alpha=alpha,
                                )
                if k == subgoal_id:
                    draw_object_and_pusher(ax=axes[0,1], object_type=obj_type, \
            ...
            # import pdb; pdb.set_trace()
            # plot the inner_pusher_pos_seq
            for k, inner_pusher_pos in enumerate(inner_pusher_pos_seq):
                if k == inner_idx:
                    alpha = 1.0
                else:
                    alpha = 0.8 - 0.6 / len(inner_pusher_pos_seq) * k
                axes[0,2].scatter(inner_pusher_pos[0]*action_scale, inner_pusher_pos[1]*action_scale, c='g', alpha=alpha)
            ...
            draw_object_and_pusher(ax=axes[0,1], object_type=obj_type, \
                                keypoints=new_env_state_for_debug[:num_kp * 2].reshape(-1,2), \
            ...
            draw_object_and_pusher(ax=axes[0,1], object_type=obj_type, \
                                keypoints=new_env_state_for_debug[num_kp * 2: state_dim].reshape(-1,2), \
                                # pusher=new_env_state_for_debug[state_dim:state_dim+2], \
                                color='mediumpurple', \
                                alpha=1.0,
                                # pusher_color='gold'
            ...
            for j in range(horizon):
                print(f"horizon: {horizon}, j: {j}, act: {action_sequence[j, 0]*action_scale, action_sequence[j, 1]*action_scale}")
                ...
                draw_object_and_pusher(ax=axes[0,0], object_type=obj_type, \
                                keypoints=state_scale*state_sequence[j][:num_kp * 2].reshape(-1,2), \
                ...
                draw_object_and_pusher(ax=axes[0,0], object_type=obj_type, \
                                keypoints=state_scale*state_sequence[j][num_kp * 2: state_dim].reshape(-1,2), \
                                # pusher=[pusher_pos_for_plt[0], pusher_pos_for_plt[1]], \
                                color='deepskyblue', \
                                alpha=alpha, \
                                # pusher_color='k'
    ...
    if not enable_real and save_result:
        ...
        env.save_gif(vis_file_name_prefix + f"_ori_{method_type}_{round(actual_step_cost, 3)}.gif")
...
def reach_pusher_pos(curr_pusher_pos, target_pusher_pos):
    return np.linalg.norm(curr_pusher_pos - target_pusher_pos) < 0.07 # mm for merging_L
...
# reward function for mppi
# [n_sample, n_look_ahead, state_dim], [n_sample, n_look_ahead, action_dim] -> [n_sample]
def reward_local(state_mppi, action_mppi, reward_config):
    device = action_mppi.device
    action_scale, state_dim = reward_config["action_scale"], reward_config["state_dim"]
    enable_latent, cost_norm, only_final_cost = reward_config["enable_latent"], reward_config["cost_norm"], reward_config["only_final_cost"]
    target_state, pusher_pos = reward_config["target_state"], reward_config["pusher_pos"]
    ...
    # step_weight = torch.arange(1, n_look_ahead + 1, (final_weight - init_weight)/n_look_ahead, device=device).float() / n_look_ahead
    step_weight = torch.arange(1, n_look_ahead + 1, device=device).float() / n_look_ahead
    step_weight = torch.flip(step_weight, dims=(0,))
    costs = (cost_seqs*step_weight).sum(dim=-1)+penalty_sum if not only_final_cost else cost_seqs[:, -1] + penalty_sum
    # print("penalty: ", penalty.max().item(), penalty.min().item())
    # print("lowest cost: ", costs.min().item(), "highest cost: ", costs.max().item())
    rewards = -costs
