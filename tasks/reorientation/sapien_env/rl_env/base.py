...
from tasks.reorientation.sapien_env.kinematics.kinematics_helper import PartialKinematicModel
from tasks.reorientation.sapien_env.utils.common_robot_utils import load_robot, generate_arm_robot_hand_info, \
    FreeRobotInfo, ArmRobotInfo
from tasks.reorientation.sapien_env.utils.random_utils import np_random
...
class BaseRLEnv(BaseSimulationEnv, gym.Env):
    ...
    def setup(self, robot_name):
        ...
        self.ee_link_name = self.kinematic_model.end_link_name # 'link6'
        ...
        self.rl_step = self.arm_sim_step_qpos
    ...
    def arm_sim_step_qpos(self, action: np.ndarray):
        # action: [7], 6 qpos of arm, 1 for gripper. 0:close, 0.75:open
        target_qpos_arm = action[:self.arm_dof]
        current_qpos_arm = self.robot.get_qpos()[:self.arm_dof]
        arm_qvel = target_qpos_arm-current_qpos_arm
        arm_qvel = np.clip(arm_qvel, -np.pi , np.pi )
        target_qvel_arm = arm_qvel

        # self.right_joint.set_drive_target(action[self.arm_dof])
        # self.left_joint.set_drive_target(action[self.arm_dof])
        for i in range(self.arm_dof):
            self.robot.get_active_joints()[i].set_drive_target(target_qpos_arm[i])
            # self.robot.get_active_joints()[i].set_drive_velocity_target(
            #     target_qvel_arm[i])

        for i in range(self.arm_dof, self.arm_dof+6):
            self.robot.get_active_joints()[i].set_drive_target(action[self.arm_dof])

        self.qvel = target_qvel_arm
    ...
    def step(self, action: np.ndarray):
        self.rl_step(action) # arm_sim_step_qpos
        self.update_cached_state() # ReorientateRLEnv().update_cached_state
