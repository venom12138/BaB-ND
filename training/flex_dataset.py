...
def load_pairs(pairs_path, episode_range):
    ...
    for episode_idx in episode_range:
        traj_path = os.path.join(pairs_path, f'{episode_idx}.p')
        with open(traj_path, 'rb') as f:
            traj = pickle.load(f)
        traj = np.array(traj)
        episodes = np.ones((traj.shape[0], 1, traj.shape[-1])) * episode_idx
        pairs = np.concatenate([episodes, traj], axis=1)
...
class FlexDynamicsDataset(Dataset):
    ...
    def __getitem__(self, i):
        # time1 = time.time()
        episode_idx = self.pair_lists[i][0][0].astype(int)
        pair = self.pair_lists[i][1:].astype(int) # (n_his+n_roll, 2)
        ...
        for i in range(len(pair)):
            frame_idx_pair = pair[i]
            obj_kp_step = []
            tool_kp_step = []
            for frame_idx in frame_idx_pair:
                obj_kp, tool_kp = extract_kp_single_frame_by(data_dir, episode_idx, frame_idx)
                obj_kp_step.append(obj_kp)
                tool_kp_step.append(tool_kp)
            obj_kps.append(obj_kp_step)
            tool_kps.append(tool_kp_step)

        obj_kp_start = obj_kps[n_his-1][0] # start frame keypoints of the object
        ...
        for fi in range(n_his+n_future):
            # downsample
            obj_kp_fi = [[obj_kps[fi][0][j][fps_idx], obj_kps[fi][1][j][fps_idx]] for j, fps_idx in enumerate(fps_idx_list)]
            obj_kp_fi = np.concatenate(obj_kp_fi, axis=0) # (2, N, 3)
            obj_kp_s, obj_kp_e = obj_kp_fi
            obj_kp_s = pad(obj_kp_s, max_nobj)
            obj_kp_e = pad(obj_kp_e, max_nobj)
            tool_kp_s, tool_kp_e = tool_kps[fi]
            # print(f"obj_kp_s: {obj_kp_s.shape}, obj_kp_e: {obj_kp_e.shape}, tool_kp_s: {tool_kp_s.shape}, tool_kp_e: {tool_kp_e.shape}")
            # states:
            states[fi, :max_nobj] = obj_kp_s
        ...
        if self.state_dim // max_nobj == 3:
            ...
            pusher_pos = tools[:, max_nobj:]
            # (n_his+n_roll, n_s, 3), the last dim has no meaning
            pusher_dir = states_delta[:, max_nobj:]
        elif self.state_dim // max_nobj == 2:
            # (n_his+n_roll, n_p, 3)
            obs = states[:, :max_nobj, [0,2]]
            # (n_his+n_roll, n_s, 3), the last dim has no meaning
            pusher_pos = tools[:, max_nobj:, [0,2]]
            # (n_his+n_roll, n_s, 3), the last dim has no meaning
            pusher_dir = states_delta[:, max_nobj:, [0,2]]            
        else:
        ...
        if np.random.uniform(0,1) < 0.2 and self.phase == 'train':
            # self.fps_idx = fps_idx_list[0]
            obs, pusher_pos, pusher_dir = self.augment_none_pushing(obs, pusher_pos, pusher_dir)
        # (n_his+n_roll, n_s, 4) or (n_his+n_roll, n_s, 6)
    ...
    def augment_none_pushing(self, obs, pusher_pos, pusher_dir):
        # ori_obs: (B, N, 2) B is traj length
        # ori_pusher_pos: (B,1,2)
        # ori_pusher_dir: (B,1,2)
        ori_obs, ori_pusher_pos, ori_pusher_dir = obs.clone(), pusher_pos.clone(), pusher_dir.clone()
        B = ori_obs.shape[0]
        obs = obs[0].unsqueeze(0).repeat(B, 1, 1) # (B, N, 2)
        pusher_lo, pusher_hi = self.config['planning']['pusher_lo'], self.config['planning']['pusher_hi']
        length_bound = self.config['planning']['action_bound']
        start_angle = np.random.uniform(0, 2 * np.pi, )
        angles = torch.tensor(np.linspace(start_angle, start_angle+2 * np.pi, 36)%(np.pi*2)) 
        directions = torch.stack([torch.cos(angles), torch.sin(angles)], dim=-1) # (36, 2)
        line_length = np.random.uniform(length_bound/2, length_bound, size=(B, )) # (B, )
        t = torch.tensor(np.linspace(0, 1, 40)) # (10,)
        # B, 36, 10, 2
        # import pdb; pdb.set_trace()
        sample_points =  ori_pusher_pos[:, :, None] + directions[None, :, None, :] * line_length[:, None, None, None] * t[None, None, :, None]
        # final_pusher_pos = ori_pusher_pos[:, :] + directions[None, :, :] * line_length[:, None, None] # B, 36, 2
        # sample_points[:, :, None, :] B, 36, N, 10, 2
        # obs[:, None, :, None :] B, 36, N, 10, 2
        # distances: # B, 36, N, 10
        distances = torch.sqrt(torch.sum((sample_points[:, :, None, :] - obs[:, None, :, None, :])**2, dim=-1)) # B, 36, N, 10
        min_distances = torch.min(distances, dim=-1).values # B, 36, N
        min_distances = torch.min(min_distances, dim=-1).values # B, 36
        valid_directions = (min_distances > 0.2).to(torch.int32) # B, 36
        # check validity
        if not torch.max(valid_directions, dim=-1).values.min() == 1:
            return ori_obs, ori_pusher_pos, ori_pusher_dir
        first_valid_indices = []
        for i in range(B):
            position_i = torch.argwhere(valid_directions[i] == 1).numpy().flatten()
            # first_valid_indices[0].append(i)
            first_valid_indices.append(np.random.choice(position_i))
        # first_valid_indices = torch.argmax(valid_directions, dim=1) # B
        # import pdb; pdb.set_trace()
        final_pusher_dir = directions[first_valid_indices] * line_length[:, None]
        final_pusher_pos = ori_pusher_pos + final_pusher_dir.unsqueeze(1)
        
        # for i in range(B):
        #     plt.figure()
        #     plt.plot(obs[i, :, 0], obs[i, :, 1], 'bo')  # 绘制点集
        #     plt.plot([ori_pusher_pos[i, 0, 0], final_pusher_pos[i, 0, 0]], 
        #             [ori_pusher_pos[i, 0, 1], final_pusher_pos[i, 0, 1]], 'r-')  # 绘制线段
        #     plt.plot(ori_pusher_pos[i, 0, 0], ori_pusher_pos[i, 0, 1], 'yo')  # 绘制初始位置
        #     plt.plot(final_pusher_pos[i, 0, 0], final_pusher_pos[i, 0, 1], 'go')  # 绘制初始位置
        #     plt.axis('equal')
        #     plt.savefig(f"augment_{i}.png")
            # plt.show()
        # import pdb; pdb.set_trace()
        return obs, ori_pusher_pos, final_pusher_dir.unsqueeze(1)
